...
const { checkLogDependency } = require('../services/log.service');
...
router.delete('/remove/:logTypeId', async function (req, res, next) {

  const logTypeId = req.params.logTypeId;

  const dependency = await checkLogDependency(logTypeId);
  if (!dependency) {
    ...
    res.send({ message: 'Log Type is linked to one or more logs!!!' });
  }
});
