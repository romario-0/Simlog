...
/* Delete Job details */
router.delete('/remove/:jobId', async function (req, res, next) {

  const jobId = req.params.jobId;

  const dependency = await checkSimulationDependency(jobId);
  if (!dependency) {
    JobModel.findOneAndDelete({ _id: jobId, status: 'New' }, function (err, jobObj) {
      if (err) {
        res.send({ message: 'Unable to delete Job' });
      } else {
        if (jobObj) {
          res.send({ message: 'Job deleted', job: jobObj });
        } else {
          res.send({ message: 'Job already in use' });
        }

      }
    });
  } else {
    res.send({ message: 'Job is associated to a Simulation!!!' });
  }
});
