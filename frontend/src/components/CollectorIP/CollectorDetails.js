...
import { createCollector, updateCollector } from "../../services/collector.service";
...
const CollectorDetails = () => {
	...
	const saveCollector = async () => {
		if (validateForm()) {
			...
			if (collectorId) {
				handleData(await updateCollector(requestOptions));
			} else {
				handleData(await createCollector(requestOptions));
			}
		}
	}
